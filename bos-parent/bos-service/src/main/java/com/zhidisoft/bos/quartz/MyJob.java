package com.zhidisoft.bos.quartz;
...
public class MyJob {
	@Resource
	private IWorkBillService workbillServcice;
	...
	public void execute() {
		...
		try {
			//查询工单类型为新单的所有工单
			List<Workbill> list = workbillServcice.findAll();
			if(null != list && list.size() > 0){
				...
				for(Workbill workbill : list){
					...
					// 设置收件人
					InternetAddress to = new InternetAddress("627103019@qq.com");
