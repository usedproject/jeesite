...
<script type="text/javascript">
	$(function(){
		...
		$('#save').click(function(){
			var f= $('#form').form("validate");
			if(f){
				$('#form').submit();
			}
		});
		//发送ajax查询所有角色
		$.post("${pageContext.request.contextPath }/${pageContext.request.contextPath}/roleAction_list.action", 
		        function(data){
		          if(data.rows!="undefined" &&data.rows.length>0){
		        	  for(var i=0;i<data.rows.length;i++){
		        		  
		        	  $("#roleId").append("<input name='roleIds' type='checkbox' value='"+data.rows[i].id+"' >"+data.rows[i].name+"</input>")
		        	  }
		          }
		         }, 
		        "json");
...
<body class="easyui-layout" style="visibility:hidden;">
    ...
    <div region="center" style="overflow:auto;padding:5px;" border="false">
       <form id="form" method="post" action="${pageContext.request.contextPath }/userAction_addUser.action" >
           <table class="table-edit"  width="95%" align="center">
				...
				<tr>
	           	...
	           	<tr><td>选择角色:</td><td colspan="3" id="roleId"></td></tr>
           </table>
       </form>
	</div>
</body>
